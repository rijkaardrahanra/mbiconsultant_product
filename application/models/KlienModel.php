...
class KlienModel extends CI_Model {
    ...
    public function Insert($clienttype_id, $user, $ciphertext, $kel_id, $nmklien, $almt, $phone, $email, $start_date, $created_id)
    {
        //$sql = "CALL insertClient('".$clienttype_id."','".$user."','".$ciphertext."','".$kel_id."','".$nmklien."','".$almt."','".$phone."','".$email."','".$start_date."','".$created_id."')";
        $uniquetoken =time().uniqid(rand());
        $sql = "CALL insertClient(?,?,?,?,?,?,?,?,?,?,?)";
        $param = array($clienttype_id,$user,$ciphertext,$kel_id,$nmklien,$almt,$phone,$email,$start_date,$created_id,$uniquetoken);
    ...
    public function Update($iduser, $unm, $startdate, $groupid, $lurah, $klien, $almt, $tlp, $mail, $updater)
    {
        ...
        $param = array($iduser,$unm,$startdate,$groupid,$lurah,$klien,$almt,$tlp,$mail,$updater);
        return $this->db->query($sql,$param);
    }
    ...
    public function Delete($id)
    {
        ...
        $sql = "UPDATE usr_user SET IS_DELETED =? WHERE USR_USER_ID =?";
        $param = array('1',$id);
        return $this->db->query($sql,$param);
    }
}