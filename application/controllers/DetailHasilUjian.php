...
class DetailHasilUjian extends MY_Controller {

	public function __construct() {
		parent::__construct();
        $this->load->model(array('HasilUjianModel','PaketUjianModel', 'DetailPaketUjianModel', 'JenjangModel', 'KelasModel', 'MapelModel', 'ProgramModel', 'BabModel','ManMapelModel'));
	}

	public function index()
	{
		redirect('DetailHasilUjian/tampil','refresh');
	}

	public function tampil()
	{
		$id = $this->uri->segment(3);
        $ujian = $this->HasilUjianModel->GetARowPaketUjian($id)->result();
        if ($ujian) {
        	$data['ujian'] = $ujian[0];
