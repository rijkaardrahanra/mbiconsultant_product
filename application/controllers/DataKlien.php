...
class DataKlien extends MY_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->library('encryption');
        $this->load->model(array('KlienModel', 'TipeKlienModel', 'PaketUjianModel','SiswaModel'));
	...
	public function detailKlien()
	{
		...
		$data['siswa'] = $this->SiswaModel->getSiswaKlien($id)->result();
	...
	public function simpanTambah()
	{
		...
		$insert = $this->KlienModel->Insert($clienttype_id, $user, $pass, $kel_id, $nmklien, $almt, $phone, $email, $start_date, $created_id);
		if ($insert) {
			echo json_encode(array(
				'status' => 1,
	...
	public function simpanTambahUjian()
	{
		...
		$created_id = $this->session->userdata('logged_in')['result']->USR_USER_ID;
		$istampil = $this->input->post('istampil');

		$insert = $this->PaketUjianModel->Insert($user_id, $nmujian, $kouta, $start, $end, $istampil, $created_id);
		if ($insert) {
			echo json_encode(array(
				'status' => 1,
				'pesan' => 'Berhasil Tersimpan'
